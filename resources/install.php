...
try {
    ...
    $ip = fopen(BASE_DIR . 'install.json', 'r');
    ...
    $mysql = new PDO('mysql:host=' . $config['mysqlHost'], $config['mysqlUser'], $config['mysqlPass'], array(
    ...
    $query->execute(array(
        ':cname' => $config['companyName'],
        ':murl' => 'http://' . $config['siteUrl'] . '/',
        ':mwebsite' => 'http://' . $config['siteUrl'] . '/',
        ':aurl' => '//' . $config['siteUrl'] . '/assets/'
    ));
    ...
    $mysql->prepare("INSERT INTO `users` VALUES(NULL, :uuid, :username, :email, :password, :language, :time, NULL, NULL, 1, 0, 1, 0, NULL)")->execute(array(
        ...
        ':password' => password_hash($config['adminPass'], PASSWORD_BCRYPT),
        ':language' => 'en',
        ':time' => time()
    ));
